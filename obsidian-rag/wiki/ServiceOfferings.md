# ServiceOfferings

**Last updated:** 2026-06-08  
**Tags:** #evergreen #decision  
**Related:** [[FortuneMarq]], [[GrowthRoadmap]], [[LockedDecisions]], [[SalesSystem]]

---

## Summary

FortuneMarq offers digital marketing services priced for accessibility, with a volume model that upsells retainers over time. All pricing is locked — do not re-discuss unless Jabeer initiates a review.

## Pricing Table (Locked)

| Service | Type | Price |
|---------|------|-------|
| Landing Page | One-time | ₹5,000–₹8,000 |
| Website | One-time | ₹8,000–₹20,000 |
| Google My Business (GMB) | Monthly retainer | ₹3,500/month |
| Google Ads Setup | One-time | ₹4,500 |
| Google Ads Management | Monthly retainer | ₹2,500/month + 5% of ad spend if spend exceeds ₹15,000/month |
| Meta Ads Management | Monthly retainer | ₹2,500/month + 5% of ad spend if spend exceeds ₹15,000/month |
| SEO | Monthly retainer | ₹7,000–₹15,000+/month |
| WhatsApp Marketing Setup | One-time | ₹5,000 |
| WhatsApp Marketing Management | Monthly retainer | ₹2,500/month |
| AI Automations | Custom | Custom pricing |

## Key Policy

Client always pays for their own ad spend (Google/Meta budget). FortuneMarq charges a management fee only — it does not bundle ad spend into service fees. If monthly ad spend exceeds ₹15,000, an additional 5% of the total ad spend is charged on top of the base management fee for that month.

## Payment Policy (Locked)

- Invoice issued: 1st of month
- Due: 5th of month
- 7 days overdue: Campaigns paused + WhatsApp alert to client
- 30 days overdue (website clients): Website taken offline until paid

## Upsell Paths

The natural progression for most clients:
- GMB → Google Ads (trigger: 50+ GMB calls/month)
- Website → GMB (trigger: website go-live)
- Google Ads → Meta Ads (trigger: after 3 months of Google results)
- Any retainer → WhatsApp Marketing (trigger: client wants lead nurturing)

## Not Offered Yet

- Social Media Management (decided: revisit after 3 months)
- Performance-based pricing/guarantees (decided: revisit after 3 months with results data)

## Open Questions

- [ ] When to raise prices for new clients? (after 3 months + portfolio)

## Sources

- [[raw/2026-04-28_business-model-pricing]]
- [[raw/2026-04-28_locked-decisions]]
