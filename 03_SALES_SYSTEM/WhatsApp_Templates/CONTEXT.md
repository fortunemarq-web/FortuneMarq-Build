...
**Last Updated:** 2026-07-08 | **Status:** COMPLETE & LIVE. **Copy refresh (2026-07-08):** two log-outcome templates were reworded to sound less robotic — `follow_back` ("No problem at all…" → "As promised…") and `not_interested` ("No worries at all…" → "Thanks for hearing me out today…"). Both edits were **submitted to Meta via the Graph API and are PENDING review**; the prior approved copy keeps sending until Meta approves the edits. `templates_final.json` + `01_CRM_AND_TOOL/fmos/WHATSAPP_TEMPLATES_FINAL.md` were updated to match. The canonical definitions live in **`FMOS_Template_Data/templates_final.json`** — **33 templates, all Meta-approved** (incl. `direct_report_type_a/b/c/d` + `daily_report`). They are registered into Meta + FMOS via the "Register 33 Meta Templates" button at `/admin/whatsapp-templates` (`01_CRM_AND_TOOL/fmos/app/admin/whatsapp-templates/actions.ts`; upserts the inline `TEMPLATES` array; skips already-approved; safe to re-run). The old `bulk_create_whatsapp_templates.py` script is removed. The older split JSON drafts (`curiosity_templates.json`, `bot_reply_templates.json`, etc.) are **superseded** — see the historical inventory below.
...
|---|---|---|
| **`templates_final.json`** | **33 (all Meta-approved)** | **Source of truth.** Meta create-payload format; positional `{{n}}` params; marketing templates carry a STOP footer. Mirrors `01_CRM_AND_TOOL/fmos/WHATSAPP_TEMPLATES_FINAL.md`. Owner-local (loaded via `WA_TEMPLATES_JSON`; not committed); the committed defs are the inline `TEMPLATES` array in `01_CRM_AND_TOOL/fmos/app/admin/whatsapp-templates/actions.ts`, registered via the "Register 33 Meta Templates" button. |
