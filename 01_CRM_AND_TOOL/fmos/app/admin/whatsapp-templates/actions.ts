...
// ─────────────────────────────────────────────────────────────────────────────
// Meta-approval WhatsApp templates — SOURCE OF TRUTH is the reviewed+submitted set
// in 03_SALES_SYSTEM/WhatsApp_Templates/FMOS_Template_Data/templates_final.json
// (33 templates as of 2026-06-16). We read that file rather than hardcoding, so the
// FMOS registry never drifts from what was actually submitted to Meta. Registered as
// a reference library so the automation rule-builder's template picker can offer them.
//
// NOTE: direct_report_type_a/b/c/d and daily_report are managed separately (already
// approved, not in this JSON) — they are intentionally untouched here.
// ─────────────────────────────────────────────────────────────────────────────
...
/** Resolve + read templates_final.json. Tries WA_TEMPLATES_JSON, then the repo-relative
 *  path (works from the main checkout), then the absolute main-checkout path (works from
 *  a worktree where 03_SALES_SYSTEM isn't present). Returns the parsed template array. */
function loadFinalTemplates(): FinalTemplate[] {
  const candidates = [
    process.env.WA_TEMPLATES_JSON,
    resolve(process.cwd(), "../../03_SALES_SYSTEM/WhatsApp_Templates/FMOS_Template_Data/templates_final.json"),
    "/Users/fortunemarq/FortuneMarq-Build/03_SALES_SYSTEM/WhatsApp_Templates/FMOS_Template_Data/templates_final.json",
  ...
  for (const path of candidates) {
    try {
      ...
      if (Array.isArray(list) && list.length) return list as FinalTemplate[];
    } catch {
      // try the next candidate
    }
  }
  throw new Error(
    "templates_final.json not found. Set WA_TEMPLATES_JSON or place it at 03_SALES_SYSTEM/WhatsApp_Templates/FMOS_Template_Data/templates_final.json"
  );
}

/** Channel grouping (DB `category`) inferred from the template name. */
function channelFor(name: string): "LEAD_CLIENT" | "ADMIN" | "STAFF" {
  if (name === "admin_alert") return "ADMIN";
  if (name === "staff_alert") return "STAFF";
  return "LEAD_CLIENT";
}

/** Distinct {{n}} placeholder count in a body. */
function paramCount(body: string): number {
  const nums = new Set((body.match(/\{\{(\d+)\}\}/g) || []).map((m) => m.replace(/\D/g, "")));
  return nums.size;
}
...
export async function seedMetaTemplates(): Promise<{
  ...
  let templates: FinalTemplate[];
  try {
    templates = loadFinalTemplates();
  } catch (err: any) {
    return { success: false, seeded, skipped, error: err.message };
  }
  ...
  try {
    for (const t of templates) {
      ...
      // variables = the example samples when present, else generic positional names.
      const examples = Array.isArray(t.example) ? t.example : [];
      const variables = examples.length
        ? examples
        : Array.from({ length: paramCount(t.body) }, (_, i) => `{{${i + 1}}}`);

      // Upsert (PK = id = name) so re-running refreshes copy that changed in the JSON.
      const { error } = await supabase.from("whatsapp_templates").upsert(
        {
          id: t.name,
          name: t.name,
          category: channelFor(t.name),
          lead_type: null,
          variables,
          content: t.body,
      ...
      if (error) {
        console.error(`Failed to register Meta template ${t.name}:`, error.message);
      } else {
        seeded++;
      }
    }
