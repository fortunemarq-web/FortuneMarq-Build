...
- **AI fallback, in-language** — `lib/bot/engine.ts`: free-text still goes to the AI (`generateBotReply` / `runBot`), which now **replies in the lead's `wa_lang`** (or mirrors the language the customer wrote in, incl. Kanglish). The static KB system prompt is now sent with **Anthropic prompt caching** to cut reply latency + cost.
