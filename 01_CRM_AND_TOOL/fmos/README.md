...
- **Delivery-failure visibility** — `whatsapp_logs.delivery_error` column added; the status webhook now records the Meta error code/reason on a failed delivery receipt instead of a bare `failed`.
