...
import { useConsent, SiteConsentBanner } from "@/components/site/site-consent";
...
export default function SiteAnalytics() {
  ...
  const hasTrackers = !!(ga4 || pixel || clarity);
  const consent = useConsent();

  if (!hasTrackers) return null; // fully inert without env

  return (
    <>
      {/* Trackers load ONLY after the visitor accepts (DPDP/GDPR). */}
      {consent === "granted" && (
        <>
          {ga4 && (
            <>
      ...
      <SiteConsentBanner hasTrackers={hasTrackers} />
    </>
  );
}
