      ...
      <div className="flex min-h-[calc(100vh-120px)] flex-col md:h-[calc(100vh-120px)]">
        ...
        <div className="flex flex-1 items-center justify-center p-6">
          <div className="text-center">
            <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-2xl bg-[#42CA80] text-white shadow-sm transition-all duration-150 hover:bg-[#35A66A] hover:shadow active:scale-[0.98] active:bg-[#2d9960] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#42CA80] focus-visible:ring-offset-2 /20">
              <BarChart3 className="h-10 w-10 text-[#42CA80]" />
            </div>
            <h2 className="text-2xl font-bold text-slate-900">Session Stats</h2>
            <div className="mt-8 flex gap-6 sm:gap-8">
              <div className="text-center">
                <p className="text-4xl font-mono tabular-nums font-bold text-slate-900 sm:text-4xl">{stats.calls}</p>
                <p className="mt-1 text-xs text-slate-600 sm:text-sm">Calls Made</p>
              </div>
              <div className="text-center">
                <p className="text-4xl font-mono tabular-nums font-bold text-[#42CA80] sm:text-4xl">{stats.qualified}</p>
                <p className="mt-1 text-xs text-slate-600 sm:text-sm">Qualified</p>
              </div>
              <div className="text-center">
                <p className="text-4xl font-mono tabular-nums font-bold text-blue-400 sm:text-4xl">{stats.contacted}</p>
                <p className="mt-1 text-xs text-slate-600 sm:text-sm">Follow-ups</p>
              </div>
            </div>
          </div>
        </div>
  ...
  if (!currentLead) {
    return (
      <div className="flex min-h-[calc(100vh-120px)] flex-col md:h-[calc(100vh-120px)]">
        ...
        <div className="flex flex-1 items-center justify-center p-6">
          <div className="text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[#42CA80] text-white shadow-sm transition-all duration-150 hover:bg-[#35A66A] hover:shadow active:scale-[0.98] active:bg-[#2d9960] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#42CA80] focus-visible:ring-offset-2 /20">
              <CheckCircle2 className="h-8 w-8 text-[#42CA80]" />
            </div>
            <h2 className="text-xl font-bold text-slate-900">Queue Complete!</h2>
  ...
  return (
    <div className="flex min-h-[calc(100vh-120px)] flex-col md:h-[calc(100vh-120px)] md:overflow-hidden">
      ...
      <div className="flex flex-col gap-2 border-b border-slate-200 bg-slate-50 px-3 py-2 sm:flex-row sm:items-center sm:justify-between sm:px-4">
        <div className="flex items-center gap-3 sm:gap-4">
          <span className="text-xs text-slate-600 sm:text-sm">
            Lead <span className="font-mono text-slate-900">{currentIndex + 1}</span> of{" "}
            <span className="font-mono text-slate-900">{queue.length}</span>
          </span>
          <div className="hidden h-4 w-px bg-white sm:block" />
          <div className="relative">
            <button
              onClick={() => setShowFilterMenu(!showFilterMenu)}
              className="flex items-center gap-1.5 rounded-lg bg-white px-2.5 py-1.5 text-xs text-slate-900 transition-colors hover:bg-slate-100 sm:gap-2 sm:px-3 sm:text-sm"
            >
              <Filter className="h-3 w-3 text-slate-600 sm:h-3.5 sm:w-3.5" />
              <span className="max-w-[100px] truncate sm:max-w-none">{filterLabels[filter]}</span>
              <ChevronDown className="h-3 w-3 text-slate-600 sm:h-3.5 sm:w-3.5" />
            </button>
            {showFilterMenu && (
              <div className="absolute left-0 top-full z-20 mt-1 w-44 rounded-lg border border-slate-200 bg-slate-50 py-1 shadow-xl sm:w-48">
                {(Object.keys(filterLabels) as FilterType[]).map((f) => (
                  <button
                    ...
                    className={clsx(
                      "w-full px-3 py-2 text-left text-xs transition-colors hover:bg-white sm:text-sm",
                      filter === f ? "text-[#42CA80]" : "text-slate-900"
                    )}
                  >
                    {filterLabels[f]}
                  </button>
      ...
      <div className="flex flex-1 flex-col overflow-y-auto md:flex-row md:overflow-hidden">
        ...
        <div className="flex w-full flex-col bg-slate-50 md:w-3/5 md:border-r md:border-slate-200">
          {/* Lead Header */}
          <div className="border-b border-slate-200 p-4 sm:p-6">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1.5 flex items-center gap-2 text-slate-500 sm:mt-2">
          ...
          <div className="flex-shrink-0 border-b border-slate-200 p-4 sm:p-6">
            <a
            ...
            >
              <Phone className="h-7 w-7 text-slate-900 sm:h-8 sm:w-8" />
              <span className="text-2xl font-bold tracking-wider text-slate-900 sm:text-4xl font-mono tabular-nums">
          ...
          {/* Context Info */}
          <div className="flex-shrink-0 border-b border-slate-200 p-3 sm:p-4">
            <div className="flex flex-wrap gap-2 sm:gap-3">
              {currentLead.industry && (
                <div className="flex items-center gap-1.5 rounded-lg bg-white px-2.5 py-1.5 sm:gap-2 sm:px-3 sm:py-2">
                  <Building2 className="h-3.5 w-3.5 text-purple-400 sm:h-4 sm:w-4" />
                  <span className="text-xs text-slate-900 sm:text-sm">{currentLead.industry}</span>
                </div>
              )}
              {currentLead.city && (
                <div className="flex items-center gap-1.5 rounded-lg bg-white px-2.5 py-1.5 sm:gap-2 sm:px-3 sm:py-2">
                  <MapPin className="h-3.5 w-3.5 text-blue-400 sm:h-4 sm:w-4" />
                  <span className="text-xs text-slate-900 sm:text-sm">{currentLead.city}</span>
                </div>
              )}
              {currentLead.email && (
                <div className="flex items-center gap-1.5 rounded-lg bg-white px-2.5 py-1.5 sm:gap-2 sm:px-3 sm:py-2">
                  <Tag className="h-3.5 w-3.5 text-amber-400 sm:h-4 sm:w-4" />
                  <span className="max-w-[150px] truncate text-xs text-slate-900 sm:max-w-none sm:text-sm">
                    {currentLead.email}
                  </span>
                </div>
              )}
          ...
          {currentLead.notes && (
            <div className="flex-shrink-0 border-b border-slate-200 p-3 sm:p-4">
              <div className="flex items-start gap-2">
                <History className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-slate-600 sm:h-4 sm:w-4" />
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] font-medium uppercase tracking-wider text-slate-600 sm:text-xs">
                    Previous Notes
                  </p>
                  <p className="mt-1 text-xs text-slate-500 sm:text-sm">{currentLead.notes}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="hidden flex-1 p-4 md:block">
            <textarea
              ...
              className="h-20 w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 placeholder-[#666] focus:border-[#42CA80]/50 focus:outline-none"
            />
            <div className="mt-3 flex flex-wrap gap-2">
              {QUICK_TAGS.map((tag) => (
                <button
                  ...
                  className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-500 transition-colors hover:border-[#42CA80]/50 hover:text-slate-900"
                >
                  {tag}
                </button>
              ))}
        ...
        <div className="flex w-full flex-col bg-slate-50 p-4 sm:p-6 md:w-2/5">
          ...
          <div className="mb-4 md:hidden">
            <textarea
              ...
              className="h-16 w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 placeholder-[#666] focus:border-[#42CA80]/50 focus:outline-none"
            />
            <div className="mt-2 flex flex-wrap gap-1.5">
              {QUICK_TAGS.slice(0, 3).map((tag) => (
                <button
                  key={tag}
                  onClick={() => addQuickTag(tag)}
                  className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] text-slate-500 transition-colors hover:border-[#42CA80]/50 hover:text-slate-900"
                >
                  {tag}
                </button>
              ))}
          ...
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-600 sm:mb-4 sm:text-sm">
            Call Outcome
          </h2>
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3">
            ...
            >
              ...
              <kbd className="absolute right-1.5 top-1.5 hidden rounded bg-[#42CA80] text-white shadow-sm transition-all duration-150 hover:bg-[#35A66A] hover:shadow active:scale-[0.98] active:bg-[#2d9960] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#42CA80] focus-visible:ring-offset-2 /20 px-1.5 py-0.5 text-[10px] font-mono text-[#42CA80] sm:block sm:right-2 sm:top-2">
                1
              </kbd>
            </button>
            ...
            <button
              onClick={() => handleOutcome("disqualified")}
              disabled={isProcessing}
              className="group relative flex flex-col items-center justify-center rounded-xl border-2 border-slate-200 bg-white/50 p-3 transition-all hover:border-slate-300 hover:bg-white active:scale-[0.98] disabled:opacity-50 sm:p-5"
            >
              <PhoneOff className="h-6 w-6 text-slate-600 sm:h-8 sm:w-8" />
              <span className="mt-1.5 text-sm font-semibold text-slate-500 sm:mt-2">Wrong #</span>
              <span className="mt-0.5 text-[10px] text-slate-600 sm:mt-1 sm:text-xs">Invalid</span>
              <kbd className="absolute right-1.5 top-1.5 hidden rounded bg-white px-1.5 py-0.5 text-[10px] font-mono text-slate-600 sm:block sm:right-2 sm:top-2">
                5
              </kbd>
            </button>
          ...
          <div className="mt-4 sm:mt-auto sm:pt-6">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 sm:p-4">
              ...
              <div className="mt-2 flex justify-between sm:mt-3">
                <div className="text-center">
                  <p className="text-lg font-bold text-slate-900 sm:text-xl">{stats.calls}</p>
                  <p className="text-[9px] text-slate-600 sm:text-[10px]">Calls</p>
                </div>
                <div className="text-center">
                  <p className="text-lg font-bold text-[#42CA80] sm:text-xl">{stats.qualified}</p>
                  <p className="text-[9px] text-slate-600 sm:text-[10px]">Hot</p>
                </div>
                <div className="text-center">
                  <p className="text-lg font-bold text-blue-400 sm:text-xl">{stats.contacted}</p>
                  <p className="text-[9px] text-slate-600 sm:text-[10px]">Follow-up</p>
                </div>
              </div>
            </div>
          </div>
...
function TopNav({
  ...
  return (
    <div className="flex items-center gap-0.5 overflow-x-auto border-b border-slate-200 bg-slate-50 px-2 sm:gap-1 sm:px-4">
      {tabs.map((tab) => (
        <button
          key={tab.key}
