...
export default function GMBChecklist({ items }: { items: any[] }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-full">
      <div className="px-6 py-4 flex items-center justify-between border-b border-slate-100 bg-slate-50/50">
        <h3 className="text-sm font-bold uppercase tracking-widest text-slate-900">
          Optimization Checklist
        </h3>
        <span className="text-xs font-bold text-[#42CA80]">
      ...
      <div className="h-1.5 w-full bg-slate-100">
        <div 
          className="h-full bg-[#42CA80] transition-all duration-500"
        ...
        {localItems.map(item => (
          <label 
            ...
            className={`flex items-start gap-3 p-3 rounded-lg border transition-all cursor-pointer ${
              item.is_completed ? 'border-emerald-100 bg-emerald-50/30 opacity-75' : 'border-slate-100 hover:bg-slate-50 hover:border-slate-200'
            }`}
          >
            <button 
              type="button"
