    ...
    if (!loading && candidates.length === 0) {
        return (
            <div className="p-8 text-center">
                <div className="mx-auto w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mb-4 border border-slate-200">
                    <Check className="h-8 w-8 text-green-500" />
                </div>
                <h2 className="text-xl font-bold text-slate-900">No duplicates found</h2>
                <p className="text-slate-500 mt-2 mb-6">Your data looks clean!</p>
                <button onClick={runScan} disabled={scanning} className="px-4 py-2 bg-slate-100 hover:bg-zinc-700 text-slate-900 rounded-lg flex items-center gap-2 mx-auto disabled:opacity-50">
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <button onClick={runScan} disabled={scanning} className="text-sm px-3 py-1.5 bg-slate-100 hover:bg-zinc-700 text-slate-700 rounded border border-slate-300 flex items-center gap-2 disabled:opacity-50">
            ...
            <div className="grid gap-4">
                {candidates.map((cand) => (
                    <div key={cand.id} className="bg-slate-50 border border-[#222] rounded-xl p-4">
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => handleIgnore(cand.id)} className="px-3 py-1.5 rounded text-xs hover:bg-slate-100 text-slate-500 hover:text-slate-900 transition-colors">Ignore</button>
...
function DuplicateCard({ lead, label }: { lead: any, label: string }) {
    if (!lead) return <div className="p-4 rounded-lg bg-slate-200/50 border border-slate-200 text-slate-500 text-sm">Lead Removed</div>;
    return (
        <div className="p-4 rounded-lg bg-slate-200/30 border border-slate-200 space-y-2">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">{label}</p>
            <h3 className="text-lg font-bold text-slate-900 truncate">{lead.company_name}</h3>
            <div className="space-y-1 text-sm text-slate-600">
                {lead.city && <p className="flex items-center gap-2"><span className="w-4 flex text-slate-600">📍</span> {lead.city}</p>}
                {lead.phone && <p className="flex items-center gap-2"><span className="w-4 flex text-slate-600">📞</span> {lead.phone}</p>}
                {lead.email && <p className="flex items-center gap-2"><span className="w-4 flex text-slate-600">✉️</span> {lead.email}</p>}
                <span className="inline-block mt-2 px-2 py-0.5 bg-slate-100 rounded text-xs text-slate-700">{lead.status}</span>
            </div>
        </div>
    );
}
