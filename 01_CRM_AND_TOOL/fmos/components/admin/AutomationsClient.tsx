...
export default function AutomationsClient({ initialRules }: { initialRules: AutomationRule[] }) {
  ...
  const [form, setForm] = useState({ name: "", description: "", trigger: "lead_stage_change", entity_type: "lead", priority: "5" });
  ...
  const createRule = async () => {
    ...
    const { data, error } = await (supabase as any).from("automation_rules")
      .insert({ name: form.name, description: form.description || null, trigger: form.trigger, entity_type: form.entity_type, priority: Number(form.priority) || 5, is_enabled: false, actions: [], conditions: [] })
      .select().single();
    if (error) { toast.error("Failed to create rule", error.message); }
    else {
      setRules((prev) => [...prev, data]);
      setShowNew(false);
      setForm({ name: "", description: "", trigger: "lead_stage_change", entity_type: "lead", priority: "5" });
    ...
    <div className="space-y-4">
      ...
      {showNew ? (
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-3">
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={() => setShowNew(false)} className="flex-1 rounded-lg border border-slate-200 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">Cancel</button>
