...
export default function TaskChecklist({ taskId, initialItems }: TaskChecklistProps) {
    ...
    return (
        <div className="mt-6 border-t border-slate-200 pt-4">
            ...
            {/* Progress Bar */}
            <div className="h-1 bg-white rounded-full mb-4 overflow-hidden">
                <div className="h-full bg-[#42CA80] text-white shadow-sm transition-all duration-150 hover:bg-[#35A66A] hover:shadow active:scale-[0.98] active:bg-[#2d9960] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#42CA80] focus-visible:ring-offset-2 transition-all duration-300" style={{ width: `${progress}%` }}></div>
            </div>

            <div className="space-y-2 mb-4">
                {items.map(item => (
                    <div key={item.id} className="flex items-start gap-2 group hover:bg-white p-2 rounded transition-colors">
                        <div className="mt-0.5 cursor-move text-[#333] opacity-0 group-hover:opacity-100">
                            <GripVertical className="h-4 w-4" />
                        </div>
                        <button onClick={() => toggleItem(item)} className="mt-0.5 text-slate-600 hover:text-slate-900 transition-colors">
                            {item.is_done ? <CheckSquare className="h-4 w-4 text-[#42CA80]" /> : <Square className="h-4 w-4" />}
                        </button>
                        <span className={item.is_done ? "text-slate-600 line-through flex-1 text-sm" : "text-slate-700 flex-1 text-sm"}>
                            {item.title}
                        </span>
                        <button onClick={() => deleteItem(item.id)} className="text-slate-600 opacity-0 group-hover:opacity-100 hover:text-red-400">
            ...
            <div className="flex gap-2">
                <input
                    ...
                    className="flex-1 bg-slate-50 border border-slate-200 rounded px-3 py-1.5 text-sm text-slate-900 focus:outline-none focus:border-slate-300"
                />
                <button onClick={addItem} disabled={isAdding} className="bg-white border border-slate-300 text-slate-900 px-3 py-1.5 rounded hover:bg-slate-100">
