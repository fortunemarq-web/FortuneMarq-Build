...
export default function MilestoneManager({ projectId, initialMilestones, isClientView = false }: { projectId: string, initialMilestones: Milestone[], isClientView?: boolean }) {
    ...
    return (
        <div className="space-y-4">
            <h2 className="text-xl font-bold text-slate-900 mb-4">Milestones & Approvals</h2>

            <div className="grid gap-4">
                {milestones.map(milestone => (
                    <div key={milestone.id} className="bg-slate-50 border border-[#222] rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <Flag className={clsx("h-4 w-4", milestone.status === 'approved' ? "text-[#42CA80]" : "text-slate-900")} />
                            ...
                            {milestone.due_date && <p className="text-xs text-slate-600 mt-2 flex items-center gap-1"><Clock className="h-3 w-3" /> Due: {new Date(milestone.due_date).toLocaleDateString()}</p>}
                        </div>
                                    ...
                                    {isClientView && (
                                        <>
                                            ...
                                            <button
                                                ...
                                                disabled={loading}
                                                className="px-4 py-2 bg-[#42CA80] text-white shadow-sm transition-all duration-150 hover:bg-[#35A66A] hover:shadow active:scale-[0.98] active:bg-[#2d9960] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#42CA80] focus-visible:ring-offset-2 text-white text-sm font-bold rounded-lg "
                                            >
                                                Approve Milestone
                                            </button>
                                        </>
