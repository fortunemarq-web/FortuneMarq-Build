...
> - **FMOS app is DEPLOYED and live** (Vercel, `fmos.fortunemarq.com`). The earlier "gated on QA / no auth" concerns are resolved (auth gate lives in `proxy.ts`, fail-open).
...
| Afifa | Telecaller — cold calling, outcome logging, meeting booking | 11am–5pm | Remote | Active |

> **Team is ONLY Jabeer + Afifa.** Website/delivery build work is done by **outsourced freelancers** on a per-project basis (engaged by Jabeer), not in-house staff. (Earlier drafts listed cousins "Zaid" and "Sufiyan" as website builders — they are **not** onboard; removed 2026-06-17.)
...
**What outsourced freelancers own (per project):**
- Executing website/delivery build tasks assigned in FMOS
- Following the PRD and prompts generated by the Website Brief App (when built)
...
STEP 4 — OUTREACH SEQUENCE
  ...
  Step 1: Direct Report — type-matched PDF (A/B/C/D) sent over WhatsApp immediately + action buttons
  Step 2: Replies handled by AI bot (6.1) or telecaller; follow-up call
  Step 3: Goal is booking a Google Meet with Jabeer (Google Calendar/Meet API)
...
STEP 8 — DELIVERY
  ...
  Website: Brief App generates PRD + prompts → freelancer builds → Jabeer reviews → live
...
| WhatsApp templates | 03_SALES_SYSTEM | COMPLETE & LIVE — **33 templates, all Meta-approved**. Source of truth: `01_CRM_AND_TOOL/fmos/WHATSAPP_TEMPLATES_FINAL.md` + `templates_final.json`. The old split JSONs (curiosity/bot_reply/outcome/etc.) are superseded; the "curiosity" teaser flow is removed (now the Direct Report). |
...
| Freelancer workflow | Task in FMOS → build → submit for review → Jabeer approves → proceed |
...
| Freelancer access to build tooling | 01_CRM_AND_TOOL + 02_SERVICE_DELIVERY | Decide how outsourced freelancers get scoped access (their own logins vs working under Jabeer's). |
